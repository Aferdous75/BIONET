...
- `tests/` — pytest correctness suite. Checks that all four
  ...
  zero variance).
- `Test.py` — tiny standalone `napypi` smoke check (unrelated to the
  main benchmark).
...
```
conda create -n bionets python=3.11
...
| `--repeats` | timing repeats per configuration (all N runs are timed and recorded, not best-of-N) |
